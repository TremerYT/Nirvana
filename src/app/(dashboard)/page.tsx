...
const Dashboard = () => {
  ...
  const currencyFormatter = React.useCallback(
    (value: string | number | undefined) => (
      <CountUp end={parseValue(value)} duration={2} separator="," prefix="Ksh " />
    ),
    []
  );

  const countFormatter = React.useCallback(
    (value: string | number | undefined) => <CountUp end={parseValue(value)} duration={1.5} />,
    []
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex justify-between gap-4 items-center">
        <div>
          <h1 className="text-4xl mb-2 font-bold">Good {greeting}, Jeffery</h1>
        ...
        <div className="flex-1 min-w-0">
          <Row gutter={[16, 16]}>
            {summaryCards.map((card) => (
              <Col key={card.label} xs={24} sm={12} md={8} lg={8} xl={8}>
                <SummaryCards {...card} />
              </Col>
            ))}
          </Row>
